...
Hover the mouse over the content block and the corresponding icon will appear on the left side of the content block. For format usage, please refer to Markdown Complete Demo[^1].

|Icon|Type|[Meta Type](#20200925102736-x94e40g)|
| --------------------------------------------------------------------------------| ----------------------| -----------------|
...
The unordered list block is a type [container block](#20200925102736-x94e40g).
...
The ordered list block is a type [container block](#20200925102736-x94e40g).
...
The task list block is a type [container block](#20200925102736-x94e40g).
...
The basic usage of outline notes can be realized through the list item block. The list item block is a type [container block](#20200925102736-x94e40g).
...
The block reference block is a type [container block](#20200925102736-x94e40g).

### Super block

![super-block](assets/super-block-20210512171243-ydsukhs.svg)

The super block is a type [container block](#20200925102736-x94e40g). It can be used to combine continuous content blocks in a document, and also to support horizontal typesetting.

### Documentation block

![doc](assets/doc-20210512171251-8qzw4l4.svg)

The entire document is a block, and the document block is a type [container block](#20200925102736-x94e40g).
...
[^1]: # Formatting elements
    ...
    Please refer to [HTML Block Notes](#20240427231112-lcwev4m) for more details.
...
[^2]: # Please Start Here
    ...
    - Navigate in the content block[^3] and Window and tab[^8]
    ...
    [Content Block Type](#20200924101225-k254i8g)
    ...
    Extend development[^81]

    Privacy Policy and User Agreement[^82]
...
[^3]: # Navigate in the Content Block
    ...
    ​#Note#: Mention search will only search in normal text, not in line-level elements such as bold and italics, and block-level elements such as code blocks and charts. The rule are the same as the detection rules of virtual reference[^7].
...
[^4]: # Ref Content Block
    ...
    - Use `((`​ to search in [all types of blocks](#20200924101225-k254i8g)
    - Use `[[` to search only [doc blocks](#20210104091538-clx67q4)
...
[^37]: # What is a Content Block
    ...
    Therefore, there are many content blocks [types](#20200924101225-k254i8g), and different types of content blocks are defined by formatting. #Content Block/Type#
...
[^39]: # Embed Content Block
    ...
    ((20210505164949-c085p1d 'Web Clipping'))
...
[^77]: # Glossary
    ...
    The key is used to encrypt data snapshots, there are three ways to generate[^79]. It is recommended that ordinary users use passphrase generation. The same passphrase can generate the same key on other devices to facilitate initial data synchronization.
...
[^79]: - <kbd>Import Key</kbd>: Copy the key string from another device to this paste and import
    - <kbd>Auto-generate key</kbd>: Use random text to generate the key, this method only needs to be executed once on one device, and other devices use <kbd>Import key</kbd>
    - <kbd>Generate key by passphrase</kbd>: Use a custom passphrase to generate the key, which requires the same passphrase on all devices
...
[^82]: # Privacy Policy and User Agreement
