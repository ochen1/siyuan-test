...
- The content block link established by ref[^1] is used for two-way linking within SiYuan
...
​#Note#: Mention search will only search in normal text, not in line-level elements such as bold and italics, and block-level elements such as code blocks and charts. The rule are the same as the detection rules of virtual reference[^83].
...
[^1]: # Ref Content Block
    ...
    - [Graph](#20200924101210-zwf8ags): browse forward and backward link relationships between content blocks
    - [Backlinks](#20200924101210-3qfiiat): Display the backlinks of the current content block as a text list
    ...
    - Use `((`​ to search in all types of blocks[^2]
    - Use `[[` to search only doc blocks[^82]
...
[^2]: # Content Block Type
    ...
    Hover the mouse over the content block and the corresponding icon will appear on the left side of the content block. For format usage, please refer to Markdown Complete Demo[^3].

    |Icon|Type|Meta Type[^81]|
    | --------------------------------------------------------------------------------| ----------------------| -----------------|
    ...
    The unordered list block is a type Meta Type[^81].
    ...
    The ordered list block is a type Meta Type[^81].
    ...
    The task list block is a type Meta Type[^81].
    ...
    The basic usage of outline notes can be realized through the list item block. The list item block is a type Meta Type[^81].
    ...
    The block reference block is a type Meta Type[^81].

    ### Super block

    ![super-block](assets/super-block-20210512171243-ydsukhs.svg)

    The super block is a type Meta Type[^81]. It can be used to combine continuous content blocks in a document, and also to support horizontal typesetting.

    ### Documentation block

    ![doc](assets/doc-20210512171251-8qzw4l4.svg)

    The entire document is a block, and the document block is a type Meta Type[^81].
...
[^3]: # Formatting elements
    ...
    Please refer to HTML Block Notes[^80] for more details.
...
[^4]: # Please Start Here
    ...
    - [Navigate in the content block](#20200924101200-gss5vee) and Window and tab[^5]
    ...
    In SiYuan[^33], the only important core concept is Content block[^34]. The content block can be formed through the formatting format, so that we can organize our thoughts and knowledge at the block-level granularity, and it is also convenient for reading and outputting long content.

    Content block[^34]

    all types of blocks[^2]

    ref[^1]

    [Navigate in the Content Block](#20200924101200-gss5vee)

    Conversion of Document and Heading[^39]

    Embed Content Block[^35]

    Content Block Attribute[^40]

    ## 🍱 Editor

    Markdown Complete Demo[^3]

    Optimize typography[^41]

    Outline by Heading and List[^44]
    ...
    ### Function Features[^30]

    Connect with third-party cloud storage[^16]

    Search asset content[^31]
    ...
    Extend development[^77]

    Privacy Policy and User Agreement[^78]
...
[^7]: # FAQ
    ...
    The data is saved in the workspace[^8] folder, in the workspace/data/ folder:

    - ​`assets` is used to save all inserted assets[^10]
    - ​`emojis` is used to save emoji images
    - ​`snippets` is used to save code snippets
    - ​`storage` is used to save query conditions, layouts and flashcards[^12], etc.
    - ​`templates` is used to save template snippets[^13]
    - ​`widgets` is used to save widgets[^14]
    - ​`plugins` is used to save plugins[^15]
    ...
    Although it does not support third-party sync disks, it supports Connect with third-party cloud storage[^16].
    ...
    Member&apos;s privileges[^18] can only be used after payment, please refer to [Pricing](https://b3log.org/siyuan/en/pricing.html).
...
[^8]: # Workspace
    ...
    - ​`data` is used to save data, please refer to here[^9] for the specific structure
...
[^9]: - ​`assets` is used to save all inserted assets[^10]
    - ​`emojis` is used to save emoji images
    - ​`snippets` is used to save code snippets
    - ​`storage` is used to save query conditions, layouts and flashcards[^12], etc.
    - ​`templates` is used to save template snippets[^13]
...
[^10]: # Assets
    ...
    In <kbd>Settings</kbd> - <kbd>Assets</kbd>, you can clean up unreferenced assets by one-click. If you need to retrieve files that have been deleted by mistake, please via Rollback[^11].
...
[^11]: ## Rollback through file history
...
[^14]: # Widgets
    ...
    Compared with plugins[^15], a widget is a kind of content block, which follows the indexing and rendering rules of the block. The widget is mainly oriented to content block extension, not to program function extension.
...
[^15]: # Plugins
    ...
    Compared with widgets[^14], the plugin is mainly for program function extension, not for content block extension.
...
[^16]: # Connect with third-party cloud storage
    ...
    Synchronizing data through third-party cloud storage services is also end-to-end encrypted[^17], and third-party cloud storage service providers cannot obtain our plaintext data.
...
[^18]: # Member's privileges

    Membership privileges are divided into two parts:

    - Annual subscription Cloud Services[^19]
    - One-time payment Function Features[^30]
...
[^19]: # Cloud Services
    ...
    ​#Note#: Cloud service related limitations[^29].
...
[^21]: # Data sync
    ...
    After initializing the data repo key[^22], if all goes well, the synchronization is already configured. If you are not familiar with synchronization configuration, it is recommended to open the synchronization configuration wizard by clicking the synchronization button. It is easier to configure through the wizard.
...
[^24]: # Data backup
    ...
    Data backup refers to uploading Data snapshot[^25] to the cloud.

    ## How to use

    1. After initializing the data repo key[^22], if all goes well, the backup has been configured
    ...
    5. After the snapshot of the cloud backup is downloaded to the local, you can restore the data by Rollback data snapshot[^26]
...
[^32]: ## Is SiYuan open source?

    SiYuan is completely open source, and the complete source code can be found on GitHub.


[^33]: ## Origin of the name "SiYuan"
...
[^34]: # What is a Content Block
    ...
    Therefore, there are many content blocks all types of blocks[^2], and different types of content blocks are defined by formatting. #Content Block/Type#
    ...
    At this point, we have been able to use content blocks to describe all content, so that we can use uniform usage to ref[^1] . Logically, there is no page concept, which reduces unnecessary burdens in use and allows users to focus on the content block.

    ## Naming and memo

    We can name each content block, add aliases and memo. Naming and aliases are mainly used for anti-link [mention ](#20210106204950-gjw62fp)search, and memo are used to record some inconvenient information in the content area.
    ...
    You can use Embed Content Block[^35] to search and summarize the naming and memo, such as listing the content blocks that contain the keyword `short` in the memo:
...
[^35]: # Embed Content Block

    ## Overview

    The embed content block is mainly used to summarize the required content blocks, use `{{` and `}}` to wrap SQL scripts on a single line: `{{ SELECT * FROM blocks WHERE content LIKE'%Keyword%' }}`, database table please refer to here[^36]. #Content Block/Embed#
    ...
    - ((20211129190925-mzmhh3v "Open source"))
    - [Pay to support development](https://b3log.org/siyuan/en/pricing.html)
...
[^36]: ## Table `blocks`
    ...
    |type|Content block type, please refer to here[^37]|
    |subtype|Content block subtype, please refer to here[^38]|
...
[^39]: # Conversion of Document and Heading
...
[^41]: # Optimize typography
    ...
    Before the actual execution of optimized typesetting, SiYuan will automatically back up the original document once, so that if the optimization result is not satisfied, it can be rolled back through Data history[^42].
...
[^42]: # Data history
    ...
    In addition to local data history, data snapshots are also used for cloud Data sync[^21].
    ...
    - When using Cleanup unreferenced assets[^43], a history will be generated, and the suffix of the history folder is `-clean`​
    - When using the Optimize typography[^41], a history will be generated, and the suffix of the history folder is `-format`​
...
[^43]: ## Cleanup unreferenced assets
...
[^44]: # Outline by Heading and List
...
[^52]: # Import and Export
    ...
    If the image syntax (local relative path) like `![foo](images/bar.png)` is encountered during import, SiYuan will convert `images/bar.png` into assets[^10]. But if it is `.images/bar.png`, it will not be converted, because the path files starting with `.` are hidden files, SiYuan will not process them.
...
[^54]: # Daily notes
    ...
    - Save path: the path under the notebook, support template variables[^55]
    - Template path (optional): use this template as the initial content when creating the daily note

    Tips: Use in the specified template Embed Content Block[^35] to summarize to-do items or content blocks that need to be viewed daily.
...
[^57]: # Database
    ...
    Templates can access, calculate and render the values of other fields in the same piece of data through the syntax introduced in template snippets[^13]:
...
[^63]: # Docker hosting
    ...
    More parameters can refer to here[^64]. The following is an example of a startup command: `docker run -d -v workspace_dir_host:workspace_dir_container -p 6806:6806 b3log/siyuan --workspace=workspace_dir_container`​
...
[^64]: # Kernel parameter
    ...
    Used to specify the browser access authentication password[^65], which will overwrite the authCode in conf.json after setting.
...
[^65]: In <kbd>Settings</kbd> - <kbd>About</kbd>, you can configure the browser access authentication password. Leaving it blank means that authentication is not enabled. This is more useful in open networks (such as office spaces and public networks), and only after authentication can you enter the work space.
...
[^69]: # Mobile App
    ...
    2. Synchronize it to the Android side through Data sync[^21]
...
[^70]: # Data Security
    ...
    - Availability[^71]: no data loss, keep the data complete and consistent
    ...
    SiYuan cloud synchronization use end-to-end encrypted[^17], the password is stored locally with a built-in key encryption, any third party other than the user cannot get the plaintext in the cloud data.
...
[^71]: # Data Availability Guarantee
    ...
    - Support File history[^72]
    - Support Data snapshot[^25]
    ...
    It is not enough to improve the availability of data on the local device, because the cloud Data sync[^21] is one of the rigid requirements. Data synchronization will affect local device data, so we should try our best to ensure the availability of data after synchronization:
...
[^73]: # Glossary
    ...
    The key is used to encrypt data snapshots, there are three ways to generate[^75]. It is recommended that ordinary users use passphrase generation. The same passphrase can generate the same key on other devices to facilitate initial data synchronization.
    ...
    Through the service capabilities provided by the SiYuan kernel use on browser[^6].
...
[^75]: - <kbd>Import Key</kbd>: Copy the key string from another device to this paste and import
    - <kbd>Auto-generate key</kbd>: Use random text to generate the key, this method only needs to be executed once on one device, and other devices use <kbd>Import key</kbd>
    - <kbd>Generate key by passphrase</kbd>: Use a custom passphrase to generate the key, which requires the same passphrase on all devices
...
[^77]: # Extend development

    - Theme[^51] Sample project github repo: siyuan-note/theme-sample
    - Icon[^50] Sample project github repo: siyuan-note/icon-sample
    - widgets[^14] Sample project github repo: siyuan-note/widget-sample
    - plugins[^15] Sample project github repo: siyuan-note/plugin-sample
    - template snippets[^13] Sample project github repo: siyuan-note/template-sample
...
[^78]: # Privacy Policy and User Agreement
...
[^80]: - When using HTML blocks, the code in them will be security filtered by default. If filtering is not required, please enable <kbd>Settings</kbd> - <kbd>Editor</kbd> - <kbd>Allow execution of scripts within HTML blocks</kbd>, but please confirm that the code is safe before opening it to avoid cross-site scripting attacks (XSS).
    - Because the Markdown specification has complicated parsing rules for HTML blocks, it is recommended to use `<div></div>` to wrap HTML code and remove blank lines.


[^81]: ## Content Block meta type
