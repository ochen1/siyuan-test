...
- Graph[^1]: browse forward and backward link relationships between content blocks
- Backlinks[^2]: Display the backlinks of the current content block as a text list
...
- Use `((`​ to search in all types of blocks[^4]
- Use `[[` to search only doc blocks[^85]
...
[^2]: ## Backlinks
    ...
    ​#Note#: Mention search will only search in normal text, not in line-level elements such as bold and italics, and block-level elements such as code blocks and charts. The rule are the same as the detection rules of virtual reference[^3].
...
[^4]: # Content Block Type
    ...
    Hover the mouse over the content block and the corresponding icon will appear on the left side of the content block. For format usage, please refer to Markdown Complete Demo[^5].

    |Icon|Type|Meta Type[^84]|
    | --------------------------------------------------------------------------------| ----------------------| -----------------|
    |​​​​![paragraph](assets/paragraph-20210512171139-2x8lgt5.svg)|Paragraph block|Leaf block|
    |​​​​![heading](assets/heading-20210512171149-56npt09.svg)|Heading block|Leaf block|
    |​​​​![math-block](assets/math-block-20210512171155-9w2eti5.svg)|Math Formula block|Leaf block|
    |​​​​![code-block](assets/code-block-20210512171201-pw7hrv2.svg)|Code block|Leaf block|
    |​​​​![table](assets/table-20210512171208-hgfm17w.svg)|Table block|Leaf block|
    |​​​​![embed-block](assets/embed-block-20250330183922-6b1vv79.svg)|Embed block|Leaf block|
    |​​​​![html-block](assets/html-block-20220429115636-e80k6ty.svg)|HTML block|Leaf block|
    |​​​​![unordered-list](assets/unordered-list-20210512171214-90nb6w0.svg)|Unordered List block|Container block|
    |​​​​![ordered-list](assets/ordered-list-20210512171220-ghfzj2f.svg)|Ordered List block|Container block|
    |​​​​![task-list](assets/task-list-20210512171226-br51zxn.svg)|Task list block|Container block|
    |​​​​![list-item](assets/list-item-20210512171231-2yc3wfd.svg)|List Item block|Container block|
    |​​​​![blockquote](assets/blockquote-20210512171236-9dl6ui9.svg)|Blockquote block|Container block|
    |​​​​![super-block](assets/super-block-20210512171243-ydsukhs.svg)|Super block|Container block|
    |​​​​![doc](assets/doc-20210512171251-8qzw4l4.svg)|Document block|Container block|
    ...
    The unordered list block is a type Meta Type[^84].
    ...
    The ordered list block is a type Meta Type[^84].
    ...
    The task list block is a type Meta Type[^84].
    ...
    The basic usage of outline notes can be realized through the list item block. The list item block is a type Meta Type[^84].
    ...
    The block reference block is a type Meta Type[^84].

    ### Super block

    ![super-block](assets/super-block-20210512171243-ydsukhs.svg)

    The super block is a type Meta Type[^84]. It can be used to combine continuous content blocks in a document, and also to support horizontal typesetting.

    ### Documentation block

    ![doc](assets/doc-20210512171251-8qzw4l4.svg)

    The entire document is a block, and the document block is a type Meta Type[^84].
...
[^5]: # Formatting elements
    ...
    Please refer to HTML Block Notes[^83] for more details.
...
[^6]: # Please Start Here
    ...
    - Navigate in the content block[^7] and Window and tab[^8]
    ...
    In SiYuan[^36], the only important core concept is Content block[^37]. The content block can be formed through the formatting format, so that we can organize our thoughts and knowledge at the block-level granularity, and it is also convenient for reading and outputting long content.

    Content block[^37]

    all types of blocks[^4]

    [Ref Content Block](#20200924101256-f8b1sbi)

    Navigate in the content block[^7]

    Conversion of Document and Heading[^43]

    Embed Content Block[^39]

    Content Block Attribute[^44]

    ## 🍱 Editor

    Markdown Complete Demo[^5]

    Optimize typography[^45]

    Outline by Heading and List[^48]
    ...
    ### Function Features[^33]

    Connect with third-party cloud storage[^19]

    Search asset content[^34]
    ...
    Extend development[^80]

    Privacy Policy and User Agreement[^81]

    Acknowledgements[^82]


[^7]: # Navigate in the Content Block
    ...
    - The content block link established by [ref](#20200924101256-f8b1sbi) is used for two-way linking within SiYuan
...
[^10]: # FAQ
    ...
    The data is saved in the workspace[^11] folder, in the workspace/data/ folder:

    - ​`assets` is used to save all inserted assets[^13]
    - ​`emojis` is used to save emoji images
    - ​`snippets` is used to save code snippets
    - ​`storage` is used to save query conditions, layouts and flashcards[^15], etc.
    - ​`templates` is used to save template snippets[^16]
    - ​`widgets` is used to save widgets[^17]
    - ​`plugins` is used to save plugins[^18]
    ...
    Although it does not support third-party sync disks, it supports Connect with third-party cloud storage[^19].
    ...
    Member&apos;s privileges[^21] can only be used after payment, please refer to [Pricing](https://b3log.org/siyuan/en/pricing.html).
...
[^11]: # Workspace
    ...
    - ​`data` is used to save data, please refer to here[^12] for the specific structure
...
[^12]: - ​`assets` is used to save all inserted assets[^13]
    - ​`emojis` is used to save emoji images
    - ​`snippets` is used to save code snippets
    - ​`storage` is used to save query conditions, layouts and flashcards[^15], etc.
    - ​`templates` is used to save template snippets[^16]
...
[^13]: # Assets
    ...
    In <kbd>Settings</kbd> - <kbd>Assets</kbd>, you can clean up unreferenced assets by one-click. If you need to retrieve files that have been deleted by mistake, please via Rollback[^14].
...
[^14]: ## Rollback through file history
...
[^17]: # Widgets
    ...
    Compared with plugins[^18], a widget is a kind of content block, which follows the indexing and rendering rules of the block. The widget is mainly oriented to content block extension, not to program function extension.
...
[^18]: # Plugins
    ...
    Compared with widgets[^17], the plugin is mainly for program function extension, not for content block extension.
...
[^19]: # Connect with third-party cloud storage
    ...
    Synchronizing data through third-party cloud storage services is also end-to-end encrypted[^20], and third-party cloud storage service providers cannot obtain our plaintext data.
...
[^21]: # Member's privileges

    Membership privileges are divided into two parts:

    - Annual subscription Cloud Services[^22]
    - One-time payment Function Features[^33]
...
[^22]: # Cloud Services
    ...
    ​#Note#: Cloud service related limitations[^32].
...
[^24]: # Data sync
    ...
    After initializing the data repo key[^25], if all goes well, the synchronization is already configured. If you are not familiar with synchronization configuration, it is recommended to open the synchronization configuration wizard by clicking the synchronization button. It is easier to configure through the wizard.
...
[^27]: # Data backup
    ...
    Data backup refers to uploading Data snapshot[^28] to the cloud.

    ## How to use

    1. After initializing the data repo key[^25], if all goes well, the backup has been configured
    ...
    5. After the snapshot of the cloud backup is downloaded to the local, you can restore the data by Rollback data snapshot[^29]
...
[^35]: ## Is SiYuan open source?

    SiYuan is completely open source, and the complete source code can be found on GitHub.


[^36]: ## Origin of the name "SiYuan"
...
[^37]: # What is a Content Block
    ...
    Therefore, there are many content blocks all types of blocks[^4], and different types of content blocks are defined by formatting. #Content Block/Type#
    ...
    At this point, we have been able to use content blocks to describe all content, so that we can use uniform usage to [link content block](#20200924101256-f8b1sbi) . Logically, there is no page concept, which reduces unnecessary burdens in use and allows users to focus on the content block.

    ## Naming and memo

    We can name each content block, add aliases and memo. Naming and aliases are mainly used for anti-link mention [^38]search, and memo are used to record some inconvenient information in the content area.
    ...
    You can use Embed Content Block[^39] to search and summarize the naming and memo, such as listing the content blocks that contain the keyword `short` in the memo:
...
[^39]: # Embed Content Block

    ## Overview

    The embed content block is mainly used to summarize the required content blocks, use `{{` and `}}` to wrap SQL scripts on a single line: `{{ SELECT * FROM blocks WHERE content LIKE'%Keyword%' }}`, database table please refer to here[^40]. #Content Block/Embed#
    ...
    After entering ((, the content block reference search will be tr...
...
[^40]: ## Table `blocks`
    ...
    |type|Content block type, please refer to here[^41]|
    |subtype|Content block subtype, please refer to here[^42]|
...
[^43]: # Conversion of Document and Heading
...
[^45]: # Optimize typography
    ...
    Before the actual execution of optimized typesetting, SiYuan will automatically back up the original document once, so that if the optimization result is not satisfied, it can be rolled back through Data history[^46].
...
[^46]: # Data history
    ...
    In addition to local data history, data snapshots are also used for cloud Data sync[^24].
    ...
    - When using Cleanup unreferenced assets[^47], a history will be generated, and the suffix of the history folder is `-clean`​
    - When using the Optimize typography[^45], a history will be generated, and the suffix of the history folder is `-format`​
...
[^47]: ## Cleanup unreferenced assets
...
[^48]: # Outline by Heading and List
...
[^56]: # Import and Export
    ...
    If the image syntax (local relative path) like `![foo](images/bar.png)` is encountered during import, SiYuan will convert `images/bar.png` into assets[^13]. But if it is `.images/bar.png`, it will not be converted, because the path files starting with `.` are hidden files, SiYuan will not process them.
...
[^58]: # Daily notes
    ...
    - Save path: the path under the notebook, support template variables[^59]
    - Template path (optional): use this template as the initial content when creating the daily note

    Tips: Use in the specified template Embed Content Block[^39] to summarize to-do items or content blocks that need to be viewed daily.
...
[^61]: # Database
    ...
    Templates can access, calculate and render the values of other fields in the same piece of data through the syntax introduced in template snippets[^16]:
...
[^67]: # Docker hosting
    ...
    More parameters can refer to here[^68]. The following is an example of a startup command: `docker run -d -v workspace_dir_host:workspace_dir_container -p 6806:6806 b3log/siyuan --workspace=workspace_dir_container`​
...
[^68]: # Kernel parameter
    ...
    Used to specify the browser access authentication password[^69], which will overwrite the authCode in conf.json after setting.
...
[^69]: In <kbd>Settings</kbd> - <kbd>About</kbd>, you can configure the browser access authentication password. Leaving it blank means that authentication is not enabled. This is more useful in open networks (such as office spaces and public networks), and only after authentication can you enter the work space.
...
[^73]: # Mobile App
    ...
    2. Synchronize it to the Android side through Data sync[^24]
...
[^74]: # Data Security
    ...
    - Availability[^75]: no data loss, keep the data complete and consistent
    ...
    SiYuan cloud synchronization use end-to-end encrypted[^20], the password is stored locally with a built-in key encryption, any third party other than the user cannot get the plaintext in the cloud data.
...
[^75]: # Data Availability Guarantee
    ...
    - Support File history[^76]
    - Support Data snapshot[^28]
    ...
    It is not enough to improve the availability of data on the local device, because the cloud Data sync[^24] is one of the rigid requirements. Data synchronization will affect local device data, so we should try our best to ensure the availability of data after synchronization:
...
[^77]: # Glossary
    ...
    The key is used to encrypt data snapshots, there are three ways to generate[^78]. It is recommended that ordinary users use passphrase generation. The same passphrase can generate the same key on other devices to facilitate initial data synchronization.
    ...
    Through the service capabilities provided by the SiYuan kernel use on browser[^9].


[^78]: - <kbd>Import Key</kbd>: Copy the key string from another device to this paste and import
    - <kbd>Auto-generate key</kbd>: Use random text to generate the key, this method only needs to be executed once on one device, and other devices use <kbd>Import key</kbd>
    - <kbd>Generate key by passphrase</kbd>: Use a custom passphrase to generate the key, which requires the same passphrase on all devices
...
[^80]: # Extend development

    - Theme[^55] Sample project github repo: siyuan-note/theme-sample
    - Icon[^54] Sample project github repo: siyuan-note/icon-sample
    - widgets[^17] Sample project github repo: siyuan-note/widget-sample
    - plugins[^18] Sample project github repo: siyuan-note/plugin-sample
    - template snippets[^16] Sample project github repo: siyuan-note/template-sample
...
[^81]: # Privacy Policy and User Agreement
...
[^83]: - When using HTML blocks, the code in them will be security filtered by default. If filtering is not required, please enable <kbd>Settings</kbd> - <kbd>Editor</kbd> - <kbd>Allow execution of scripts within HTML blocks</kbd>, but please confirm that the code is safe before opening it to avoid cross-site scripting attacks (XSS).
    - Because the Markdown specification has complicated parsing rules for HTML blocks, it is recommended to use `<div></div>` to wrap HTML code and remove blank lines.


[^84]: ## Content Block meta type
